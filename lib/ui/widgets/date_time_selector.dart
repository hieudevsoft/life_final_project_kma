...
class _DateTimeSelectorFormFieldState extends State<DateTimeSelectorFormField> {
  ...
  void initState() {
    ...
    if (widget.displayDefault && widget.minimumDateTime != null) {
      if (widget.type == DateTimeSelectionType.date) {
        _textEditingController.text = widget.minimumDateTime?.dateToStringWithFormat(format: "dd/MM/yyyy") ?? "";
      } else {
        _textEditingController.text = widget.minimumDateTime?.getTimeInFormat(TimeStampFormat.parse_12) ?? "";
      }
    }
  }
  ...
  Future<void> _showSelector() async {
    ...
    if (widget.type == DateTimeSelectionType.date) {
      ...
      _textEditingController.text = (date ?? _selectedDate).getTimeInFormat(TimeStampFormat.parse_12);
    }
        ...
        builder: (context, child) {
          return Theme(
            data: ThemeData.light().copyWith(
              ...
              colorScheme: ColorScheme.light(primary: context.colorScheme.primary),
